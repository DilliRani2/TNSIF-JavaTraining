...
public class OrderDetailsController {
    ...
    public ResponseEntity<Order> get(@PathVariable Integer id) {
        try {
            ...
            return new ResponseEntity<>(HttpStatus.NOT_FOUND); // Changed to NOT_FOUND to make REST sense, though originally CREATED in Sprint 1
        }
    }
    ...
    public ResponseEntity<?> delete(@PathVariable Integer id) {
        try {
            ...
            return new ResponseEntity<>(HttpStatus.NOT_FOUND); // Adjusted from original CREATED for better REST semantics
        }
    }
}
